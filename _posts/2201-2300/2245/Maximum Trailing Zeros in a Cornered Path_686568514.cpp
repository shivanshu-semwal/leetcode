    ...
    int maxTrailingZeros(vector<vector<int>>& grid) {
        ...
        for(int i = 0; i < r; i++){
            for(int j = 0; j < c; j++){
                ...
                down.first = top[r-1][j].first - top[i][j].first;
                down.second = top[r-1][j].second - top[i][j].second;
                
                right.first = left[i][c-1].first - left[i][j].first;
                right.second = left[i][c-1].second - left[i][j].second;
